...
case class SnowHousePipeStageInstrFetch(
  ...
  when (up.isFiring) {
    ...
    when (rSavedExSetPc.fire) {
      ...
      nextRegPc := rPrevRegPcThenNext /*rPrevRegPc + (cfg.instrMainWidth / 8)*/
...
case class SnowHousePipeStageExecute(
  ...
  when (doCheckHazard(0)) {
    when (myDoHaveHazard(0)) {
              ...
              if (busIdxFound) {
                ...
                when (
                  /*LcvFastAndR*/(
                    Vec[Bool](
                      !rSavedStall,
                      RegNext(next=doCheckHazard(idx), init=False),
                      RegNext(next=myDoHaveHazard(idx), init=False),
                    ).asBits.asUInt.andR
                  )
                ) {
