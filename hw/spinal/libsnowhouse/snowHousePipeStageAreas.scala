  ...
  val tgtBufWrAddr = (
    ...
    //RegNext(
      RegNext(
        ...
        (
          io.psExSetPc.branchTgtBufElem.srcRegPc //- cfg.instrSizeBytes//1
        )(myTgtBufAddrRange)
        //- 1
...
case class SnowHousePipeStageInstrFetchLcvIbus(
  ...
  io.lcvIbus.h2dBus.isWrite := False
  io.lcvIbus.h2dBus.byteSize := (
    log2Up(cfg.instrMainWidth / 8)
  )
    ...
    switch (myUpdateRegPcCondUInt) {
      ...
      is (M"10") {
        if (cfg.haveBranchPredictor) {
          ...
          //val temp = cloneOf(myPredictedNextPc)
          //when (
          //  RegNextWhen(
          //    myMainPredictCond,
          //    cond=up.isFiring,
          //    init=myMainPredictCond.getZero
          //  )
          //) {
          //  temp := (
          //    RegNextWhen(
          ...
          myBusAddr := (
            //myIbusRegPcInfo.regPc
            temp.resize(myBusAddr.getWidth)
          )
          myIbusRegPcInfo.regPc := (
            temp
            //RegNextWhen(
...
case class SnowHousePipeStageInstrFetchLastLcvIbus(
  ...
  def cfg = args.cfg
  def io = args.io
  def cIfLast = args.link
  def pIf = args.currPayload
  val up = cIfLast.up
  val down = cIfLast.down

  val myIfLastPayload = Vec.fill(2)(
    SnowHousePipePayload(cfg=cfg)
  )
  myIfLastPayload(0) := up(pIf)
  myIfLastPayload(1) := (
    //RegNext(
    //  myIfLastPayload(1),
    //  init=myIfLastPayload(1)
    //)
    myIfLastPayload(0)
  )
...
//  }
//}
//case class SnowHousePipeStageInstrFetchPostLcvIbus(
//  args: SnowHousePipeStageArgs,
//) extends Area {
//  def cfg = args.cfg
//  def io = args.io
//  def cIfPostLcvIbus = args.link
//  def up = cIfPostLcvIbus.up
//  def down = cIfPostLcvIbus.down
//  def pIf = args.prevPayload
//  def pIfPostLcvIbus = args.currPayload
//
//  val myIfPostLcvIbusPayload = Vec.fill(2)(
//    SnowHousePipePayload(cfg=cfg)
//  )
//  myIfPostLcvIbusPayload(0) := up(pIf)
//  myIfPostLcvIbusPayload(1) := myIfPostLcvIbusPayload(0)
//  up(pIfPostLcvIbus) := myIfPostLcvIbusPayload(1)
//  val myHistUpFiring = (
//    History[Bool](
//      that=True,
//      when=up.isFiring,
...
case class SnowHousePipeStageExecuteSetOutpModMemWord(
  ...
  //io.psExSetPc.branchTgtBufElem.dontPredict.setAsReg() init(False)
  val rTempBranchDontPredict = Reg(Bool(), init=False)
  rTempBranchDontPredict := (
    io.btbElemDontPredict
    || stickyTakeIrq
  )
  ...
  when (
    ...
    myTempPsExSetPcDstRegPc.foreach(item => {
      item := io.mySavedRegPcPlusInstrSize.last
    })
    io.psExSetPc.branchTgtBufElem.dstRegPc := (
      //io.mySavedRegPcPlusInstrSize.last
      //io.mySavedRegPcPlusInstrSize.last
