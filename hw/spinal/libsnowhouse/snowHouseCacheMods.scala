...
case class SnowHouseDataCache(
  ...
  val rRevLineAddrCnt = (
    Reg(SInt((rLineAddrCnt.getWidth + 1) bits))
    init(
      (
        1
        << (
  ...
  def atLastLineBusAddrCnt(addend: Int=(-1)) = (
    //rLineAddrCnt.msb
    //rRevLineAddrCnt.msb
    (rRevLineAddrCnt + addend /*- 1*/).msb// === 0x0
  )
  //def atLastRecvCnt() = (
  //  rRecvCnt.msb
  //)
  ...
  //if (isIcache) {
  //  io.bus.recvData.setAsReg() init(io.bus.recvData.getZero)
  ...
  switch (rState) {
            ...
            is (M"0--") {
              ...
              when (rdLineAttrs.dirty) {
                ...
                nextState := (
                  ...
                  State.HANDLE_RECV_LINE_FROM_BUS_PIPE_1
                )
              }
            }
            //is (M"0--") {
    ...
    is (State.HANDLE_RECV_LINE_FROM_BUS_PIPE_1) {
      nextState := (
        State.HANDLE_RECV_LINE_FROM_BUS
      )
      setLineBusAddrCntsToStart()
    ...
    is (State.HANDLE_RECV_LINE_FROM_BUS) {
      ...
      when (
        ...
        doLineWordRamWrite(
          busAddr=tempLineBusAddr,
          lineWord=Some(myD2hBus.sendData.data),
        )
        myD2hBus.ready := True
      }
    }
...
case class SnowHouseInstrCache(
  cfg: SnowHouseConfig,
  //isIcache: Boolean,
) extends Component {
  val isIcache = true
  ...
  //  //  init=lineWordRam.io.rdEn.getZero,
  //  //)
  //)
  //lineWordRam.io.rdAddr := (
  //  //0x0
  //  RegNext(
  //    next=lineWordRam.io.rdAddr,
  //    init=lineWordRam.io.rdAddr.getZero
  //  )
  //)
  lineWordRam.io.wrEn := False
  ...
  ////lineWordRam.io.rdEn := False
  ////when (
  ////  (RegNext(rState) init(State.IDLE))
  ////  =/= State.HANDLE_RECV_LINE_FROM_BUS
  ////) {
    doAllLineRamsReadSync(busAddr=io.bus.sendData.addr)
    //doLineWordRamReadSync(busAddr=io.bus.sendData.addr)
